...
export const GetTimelineUserOwner = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://ss-track.vercel.app/api/v1/owner/sorted-datebased/${userId}?date=${encodeURIComponent(formattedDate)}`, {
            headers: headers,
        })
        if (response.status) {
            dispatch(setLoading(false))
...
export const GetTimelineUserSuperAdmin = createAsyncThunk("get/timeline-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://ss-track.vercel.app/api/v1/superAdmin/sorted-datebased/${userId}?date=${encodeURIComponent(formattedDate)}`, {
            headers: headers,
        })
        if (response.status) {
            dispatch(setLoading(false))
...
export const GetOwnerTeam = createAsyncThunk("get/team-users", async (body, { getState, dispatch }) => {
    ...
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`https://ss-track.vercel.app/api/v1/owner/companies`, {
            headers: headers,
        })
        if (response.status) {
