...
function ActivityLevel(props) {
    ...
    const handleApplySettings = async (employee, check) => {
        ...
        try {
            const res = await axios.patch(`https://ss-track.vercel.app/api/v1/owner/settingsE/${employee._id}`, {
                userId: employee._id,
    ...
    async function handleApply(activityLevelTracking) {
        try {
            const res = await axios.patch(`https://ss-track.vercel.app/api/v1/superAdmin/settingsE`,
    ...
    async function getData() {
        try {
            const response = await fetch(`https://ss-track.vercel.app/api/v1/superAdmin/employees`, { headers })
            const json = await response.json();
            dispatch(getEmployess(json?.convertedEmployees))
            // json?.convertedEmployees.map(async (employee) => {
            //     const data = await axios.get(`https://ss-track.vercel.app/api/v1/superAdmin/Settings/${employee._id}`)
